...
final class TimeFormattingTests: XCTestCase {
    ...
    // MARK: - TimeFormatter.formatTimeNoSeconds

    func testFormatTimeNoSeconds_zero() {
        XCTAssertEqual(TimeFormatter.formatTimeNoSeconds(0), "0:00")
    }

    func testFormatTimeNoSeconds_59seconds() {
        // 59 seconds is less than a minute — rounds down to 0 minutes
        XCTAssertEqual(TimeFormatter.formatTimeNoSeconds(59), "0:00")
    }

    func testFormatTimeNoSeconds_60seconds() {
        XCTAssertEqual(TimeFormatter.formatTimeNoSeconds(60), "0:01")
    }

    func testFormatTimeNoSeconds_exactlyOneHour() {
        XCTAssertEqual(TimeFormatter.formatTimeNoSeconds(3600), "1:00")
    }

    func testFormatTimeNoSeconds_largeValue() {
        // 100 hours = 360000 seconds → "100:00"
        XCTAssertEqual(TimeFormatter.formatTimeNoSeconds(360000), "100:00")
    }

    // MARK: - TimeFormatter.formatTimeRemaining

    func testFormatTimeRemaining_positiveMinutesOnly() {
        // 45 minutes
        XCTAssertEqual(TimeFormatter.formatTimeRemaining(45 * 60), "45m")
    }

    func testFormatTimeRemaining_positiveHoursAndMinutes() {
        // 2 hours 30 minutes
        XCTAssertEqual(TimeFormatter.formatTimeRemaining(2 * 3600 + 30 * 60), "2h 30m")
    }

    func testFormatTimeRemaining_positiveDays() {
        // 1 day 3 hours
        XCTAssertEqual(TimeFormatter.formatTimeRemaining(1 * 86400 + 3 * 3600), "1d 3h")
    }

    func testFormatTimeRemaining_zero() {
        XCTAssertEqual(TimeFormatter.formatTimeRemaining(0), "0m")
    }

    func testFormatTimeRemaining_negative_treatedAsAbsolute() {
        // Negative intervals are abs'd — same output as positive equivalent
        XCTAssertEqual(TimeFormatter.formatTimeRemaining(-3600), "1h 0m")
    }

    // MARK: - TimeFormatter.formatDueDate

    func testFormatDueDate_returnsNonEmptyString() {
        // Build a fixed date via DateComponents so the result is locale-independent to test
        ...
        let result = TimeFormatter.formatDueDate(date)
        XCTAssertFalse(result.isEmpty)
    }

    func testFormatDueDate_containsYear() {
        var components = DateComponents()
        ...
        let date = cal.date(from: components)!
        let result = TimeFormatter.formatDueDate(date)
        XCTAssertTrue(result.contains("2030"))
    }

    // MARK: - TimeFormatter.formatTime boundary values

    func testFormatTime_exactlyOneHour() {
        XCTAssertEqual(TimeFormatter.formatTime(3600), "1:00:00")
    }

    func testFormatTime_veryLargeValue() {
        // 99 hours, 59 minutes, 59 seconds
        let interval: TimeInterval = 99 * 3600 + 59 * 60 + 59
        XCTAssertEqual(TimeFormatter.formatTime(interval), "99:59:59")
    }
}
